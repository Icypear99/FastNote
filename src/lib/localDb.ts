...
const defaultCards = (): DashboardCard[] => [
  {id: id(), cardType: 'todo-overview', cardConfig: {size: 'small'}, isVisible: true, orderNum: 1},
  {id: id(), cardType: 'today-focus', cardConfig: {size: 'small'}, isVisible: true, orderNum: 2},
  {id: id(), cardType: 'recent-notes', cardConfig: {size: 'medium'}, isVisible: true, orderNum: 3},
  {id: id(), cardType: 'quick-tools', cardConfig: {size: 'medium'}, isVisible: true, orderNum: 4},
];
...
const seedTasks = (): Task[] => [
  ...
  },
  {
    id: id(),
    title: '配置 AI 助手模型',
    description: '先使用 mock 模式，后续填入 OpenAI-compatible 配置。',
    type: 'task',
    priority: 'P1',
    status: 'todo',
    labels: ['AI'],
...
export const localDb = {
  ...
  async sendMessage(content: string, conversationId?: string) {
    ...
    const assistantMessage: AiMessage = {
      ...
      role: 'assistant',
      content:
        snapshot.settings.aiProvider === 'mock' || !snapshot.settings.aiApiKey
          ? `已收到：${content}\n\n当前处于本地 mock 模式。配置 OpenAI-compatible API 后，这里会走真实模型请求。`
          : '模型请求已提交。若桌面端 Rust 后端可用，会返回真实模型响应；Web 预览保持本地 mock 兜底。',
      createdAt: now(),
    };
